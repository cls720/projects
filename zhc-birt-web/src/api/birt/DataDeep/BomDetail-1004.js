export const bomDetailModel1004 =
{
  controlName: "BirtWorkBook",
  controlId: "BirtWorkBook_0",
  children: [
    {
      controlName: "BirtSheet",
      controlId: "BirtSheet_0",
      name: 'sheet0',//配置页,children为其算法分页，如插入分页符或A4纸数据过长换页
      pageIndex: 0,
      style: "padding:10px;",
      children: [
        {
          controlName: "BirtGridSheet",
          controlId: "BirtFormSheet_01",
          width: 705,
          style: "border-spacing:0px;border-collapse: collapse;",
          rows: {
            r1: {
              rowHeight: 40,
              tds: {
                A1: {
                  value: "BOM清单列表",
                  style: "text-align:left;vertical-align:middle;font-size:24px;font-weight:bold;",
                  colspan: 4
                },
                E1: {
                  value: "订单号：",
                  style: "text-align:right;vertical-align:bottom;font-size:18px;font-weight:bold;",
                  colspan: 3
                },
                H1: {
                  value: "HTDD-160110-1004",
                  style: "text-align:right;color:blue;vertical-align:bottom;font-size:18px;font-weight:bold;",
                  hrefs: [{
                    controlName: "HcLinkBirt",
                    controlId: "HcLinkBirt_123aa",
                    title: "数据钻取",
                    birtUrl: "/birt/advince/data-deep",
                    target: "",
                    request: "get",
                    params: {}
                  }]
                }
              }
            },
            r2: {
              rowHeight: 5,
              tds: {
                A2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                },
                B2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                },
                C2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                },
                D2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                },
                E2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                },
                F2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                },
                G2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                },
                H2: {
                  value: "",
                  style: "border-bottom: 3px solid rgb(108,202,205)"
                }
              }
            },
            r3: {
              rowHeight: 20,
              tds: {
                A3: {
                  value: "",
                  style: ""
                },
                B3: {
                  value: "",
                  style: ""
                },
                C3: {
                  value: "",
                  style: ""
                },
                D3: {
                  value: "",
                  style: ""
                },
                E3: {
                  value: "",
                  style: ""
                },
                F3: {
                  value: "",
                  style: ""
                },
                G3: {
                  value: "",
                  style: ""
                },
                H3: {
                  value: "",
                  style: ""
                }
              }
            },
            r4: {
              rowHeight: 45,
              tds: {
                A4: {
                  value: "物料名称",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                },
                B4: {
                  value: "物料编号",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                },
                C4: {
                  value: "物料颜色",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                },
                D4: {
                  value: "物料幅宽",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                },
                E4: {
                  value: "物料克重(g/㎡)",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                },
                F4: {
                  value: "算料单位",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                },
                G4: {
                  value: "算料数量",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                },
                H4: {
                  value: "算料重量(kg)",
                  style: "text-align:center;background-color:rgb(108,202,205);color:rgb(255,255,255);border-top-color:rgb(108,202,205);font-family:Microsoft YaHei UI;font-size:16px;border-right-color:rgb(255,255,255);"
                }
              }
            },
            r5: {
              rowHeight: 40,
              tds: {
                A5: {
                  value: "纯棉布",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                },
                B5: {
                  value: "bu-1001",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                },
                C5: {
                  value: "白",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                },
                D5: {
                  value: "120cm",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                E5: {
                  value: "180",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                F5: {
                  value: "m/米",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                G5: {
                  value: "100",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                H5: {
                  value: "21.6",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                }
              }
            },
            r6: {
              rowHeight: 40,
              tds: {
                A5: {
                  value: "纯棉布",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;",
                },
                B5: {
                  value: "bu-1002",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;",
                },
                C5: {
                  value: "蓝",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;",
                },
                D5: {
                  value: "120cm",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;", colspan: 1
                },
                E5: {
                  value: "180",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;", colspan: 1
                },
                F5: {
                  value: "m/米",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;", colspan: 1
                },
                G5: {
                  value: "100",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;", colspan: 1
                },
                H5: {
                  value: "21.6",
                  style: "text-align:center;background-color:rgb(243,243,243);color:rgb(68,68,68);font-family:Microsoft YaHei UI;font-size:16px;",
                }
              }
            },
            r7: {
              rowHeight: 40,
              tds: {
                A5: {
                  value: "纯棉布",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                },
                B5: {
                  value: "bu-1001",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                },
                C5: {
                  value: "黑",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                },
                D5: {
                  value: "120cm",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                E5: {
                  value: "180",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                F5: {
                  value: "m/米",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                G5: {
                  value: "50",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                  colspan: 1
                },
                H5: {
                  value: "10.8",
                  style: "text-align:center;color:rgb(68,68,68);font-family:Microsoft YaHei UI",
                }
              }
            },

            r11: {
              rowHeight: 3,
              tds: {
                A10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                },
                B10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                },
                C10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                },
                D10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                },
                E10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                },
                F10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                },
                G10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                },
                H10: {
                  value: "",
                  style: "border-bottom: 1px solid rgb(108,202,205)"
                }
              }
            },
            r12: {
              rowHeight: 28,
              tds: {
                A11: {
                  value: "",
                  style: ""
                },
                B11: {
                  value: "",
                  style: ""
                },
                C11: {
                  value: "",
                  style: ""
                },
                D11: {
                  value: "",
                  style: "text-align:center;font-weight:bold",
                },
                E11: {
                  value: "",
                  style: "text-align:center;font-weight:bold",
                },
                F11: {
                  value: "汇总：",
                  style: "text-align:center;font-weight:bold",
                },
                G11: {
                  value: "250",
                  style: "text-align:center;font-weight:bold",
                },
                H11: {
                  value: "54",
                  style: "text-align:center;font-weight:bold",
                }
              }
            }
          },
          cols: [{
            colWidth: 150
          }, {
            colWidth: 120
          }, {
            colWidth: 100
          }, {
            colWidth: 100
          }, {
            colWidth: 120
          }, {
            colWidth: 100
          }, {
            colWidth: 120
          }, {
            colWidth: 165
          }]
        }]
    }]
}
