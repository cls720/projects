 ...
 * @date 2018/12/19
 */
Vue.component("gp-table", {
	template: `	
	<table>
		<thead>		
			<template v-for="tr in theadTrs">
				<tr>
					<template v-for="th in tr">
						<th :islock="th.isLock" :isleaf="th.isLeaf" :rowspan="th.rowspan" :colspan="th.colspan" :width="th.width" :style="th.style">
							<template v-if="th.isRowFold">
								<a href="javascript:void(0);" class="buttonTag white glyphicon glyphicon-minus" :level="th.level"></a>								
							</template>
							<template v-if="th.isHaving">
								<a :href="th.havingCdionUrl" :data-param='th.param' data-toggle="dialog" data-width="400" data-height="220" data-resizable="false" 
									data-id="dialog-mask" data-mask="true" title="添加结果集过滤条件" data-title="添加结果集过滤条件" style="float:right;text-decoration:none">
		...
		<tbody>
			<template v-for="tr in tbodyTrs">
				<tr axis="{{tr.axis}}">
					<template v-for="td in tr.tds">
						<td :islock="td.isLock" :level="td.level" :dlevel="td.dlevel" :valign="td.valign" :rowspan="td.rowspan" :colspan="td.colspan" :class="td.class" :style="td.style">
							<template v-if="td.isRowFold">
								<a href="javascript:void(0);" class="buttonTag white glyphicon glyphicon-minus" :axis="td.axis" :level="td.level"></a>								
							</template>	
							{{td.desc}}
						</td>						
					</template>
