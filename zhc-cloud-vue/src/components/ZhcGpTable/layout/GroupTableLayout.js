...
jQuery.GroupTableLayout = jQuery.gtl = {
   ...
   * @param {*} dependDatas 
   */
  getDependDatas: function (recdsJa, dependDatas) {
    var retuDatas = {};
    $.each(dependDatas, function () {
      var distinctList = $.ju.getDistinctValues(recdsJa,
        this.field);
      let len = distinctList.length;
      if (len > 0) {
        retuDatas[this.field] = (len === 1 ? distinctList[0] : len)
      }
    })
    return retuDatas;
  },
   ...
   * @param dataFields
   *            String[]数据字段	
   * @param dependDatas JSONObject 依赖本分组行或列其它字段值 {groupField1:[{field:"依赖fieldId1"}]} 
   * @param dependFields
   ...
   * @return
   */
  getGroupJsonTree: function (recdsJa, retuJa, groupFields, groupFields2, dataFields, dependDatas, dependFields,
    isFirstLevel) {
    if (groupFields.length > 0) {
      ...
      $.each(jo, function (key, ja) {
        ...
        var children = $.gtl.getGroupJsonTree(ja, retuJa, subGroupFields,
          groupFields2, dataFields, dependDatas, dependFields, false);
        // 添加子分组
        if (children != null) {
          itemJo.children = children;
        }
        // 计算依赖数据
        if (dependDatas && dependDatas[fn]) {
          itemJo.dependDatas = $.gtl.getDependDatas(ja, dependDatas[fn])
        }
        // 计算分组字段依赖字段列表值
  ...
  sumGroupSubAndAllItem: function (itemJo, totalJa) {
    ...
    if (dataFieldJo == null) {
      ...
      if (val != null) {
        ...
        val = numberPlus(val, $.gtl.getJsonDoubleValue(itemJo, "value"))
        var count = 2;
  ...
  rowAddGroupTd: function (rowsJaItem, newRecd, axis, policy) {
    ...
    var gTd = {
      ...
      dependDatas: rowsJaItem.dependDatas,
