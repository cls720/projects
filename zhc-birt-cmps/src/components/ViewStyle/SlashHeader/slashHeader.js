...
jQuery.SlashHeader = {
  draw(setting) {
    ...
    if (options.initialPoint == 'top') {
      ...
      options.renderEnd = {
        'x': options.renderWidth,
        'y': options.renderHeight
      };
    } else {
      options.renderStart = {
        'x': 0,
    ...
    for (var i = 0; i < headerArr.length; i++) {
      ...
      if (options.initialPoint == 'top') {
        if (i == 0) {
          fontStyle = `left:${options.textIndent};bottom:${options.textIndent};`;
          ...
          fontStyle = `right:${options.textIndent};top: ${divY}px`;
        } else if (Math.abs(pointArr[i].x - pointArr[i - 1].x) > 0) {
          fontStyle = `left: ${pointArr[i - 1].x}px;bottom:${options.textIndent};`;
        } else if (headerArr.length == 2 && i == 1) {
          fontStyle = `right:${options.textIndent};top:${options.textIndent}`;
        }
      } else {
        if (i == 0) {
          fontStyle = `left:${options.textIndent};top:${options.textIndent};`;
        } else if (Math.abs(pointArr[i].y - pointArr[i - 1].y) > 0) {
          if (i == headerArr.length - 1) {
            var divY = 0;
          } else {
          ...
          fontStyle = `right:${options.textIndent};bottom: ${divY}px`;
        } else if (Math.abs(pointArr[i].x - pointArr[i - 1].x) > 0) {
          fontStyle = `left: ${pointArr[i - 1].x}px;top:${options.textIndent};`;
        } else if (headerArr.length == 2 && i == 1) {
          fontStyle = `right:${options.textIndent};bottom:${options.textIndent}`;
        }
      }
      var div = `<div class="${options.fontClass}" style="position:absolute; ${fontStyle}"> ${headerArr[i]}</div>`;
...
export default jQuery.SlashHeader
